<script lang="ts">
	...
	let { open = $bindable(), type, onClose, onImport, labels = {} }: Props = $props();
	...
	// Interpolate strings with parameters
	...
	<div class="modal-box max-w-2xl">
		<div class="flex items-center justify-between mb-4">
			<h3 class="font-bold text-lg">
				...
				{type === 'preamble' ? t.resolutionImportPreamble : t.resolutionImportOperative}
			</h3>
			<button
				type="button"
		...
		<p class="text-sm text-base-content/70 mb-3">
			{type === 'preamble' ? t.resolutionImportHintPreamble : t.resolutionImportHintOperative}
		</p>
		...
		<details class="collapse collapse-arrow bg-base-200 mb-4">
			<summary class="collapse-title text-sm font-medium py-2 min-h-0">
				...
				{t.resolutionImportTipsTitle}
			</summary>
			<div class="collapse-content text-sm">
				<ul class="list-disc list-inside space-y-1 text-base-content/70">
					{#if type === 'preamble'}
						<li>{t.resolutionImportTipsPreamble1}</li>
						<li>{t.resolutionImportTipsPreamble2}</li>
						<li>{t.resolutionImportTipsPreamble3}</li>
					{:else}
						<li>{t.resolutionImportTipsOperative1}</li>
						<li>{t.resolutionImportTipsOperative2}</li>
						<li>{t.resolutionImportTipsOperative3}</li>
						<li>{t.resolutionImportTipsOperative4}</li>
					{/if}
				</ul>
			</div>
		</details>
		...
		<details class="collapse collapse-arrow bg-base-200 mb-4">
			<summary class="collapse-title text-sm font-medium py-2 min-h-0">
				...
				{t.resolutionImportLLMTitle}
			</summary>
			<div class="collapse-content text-sm">
				<p class="text-base-content/70 mb-3">
					{t.resolutionImportLLMInstructions}
				</p>
				<div class="relative">
					<pre
						...
						'preamble'
							? t.resolutionImportLLMPromptPreamble
							: t.resolutionImportLLMPromptOperative}</pre>
					<button
						type="button"
					...
					>
						...
						{copied ? t.resolutionImportLLMCopied : t.resolutionImportLLMCopyPrompt}
					</button>
				</div>
			</div>
		</details>
		...
		{#if clauseCount.total > 0}
			<div class="mt-4">
				<div class="text-sm font-medium mb-2">
					{interpolate(t.resolutionImportPreview, { count: clauseCount.total })}
					{#if type === 'operative' && clauseCount.sub > 0}
						<span class="text-base-content/60">
							({clauseCount.main}
							{t.resolutionOperativeClauses}, {clauseCount.sub}
							{t.resolutionSubClauses})
						</span>
					{/if}
				</div>
		...
		<div class="modal-action">
			...
			>
				...
				{interpolate(t.resolutionImportButton, { count: clauseCount.total })}
			</button>
		</div>
	</div>
